...
最終更新: 2026-07-09（パイロットテスト・GAS 本番 URL 更新・handoff 整備）
...
| 7 | `doc/handoff/pilot-test-handoff-report.md` | 本生成前 8 件パイロット結果・DoD |
...
| GAS 本番 | 下記 URL。CORS + Opus 4.7。プロンプト修正反映済み（手動デプロイ） |
| `data/current/items.json` | **3 件**（Phase 1 サンプル） |
| A2 本生成（2,430 件） | **未着手**（パイロット再テスト待ち） |
...
│       └── pilot-test-handoff-report.md # 8 件パイロット・DoD
│
├── src/                               # PWA 本体
...
| `handoff/` | 何が終わっていて何が残っているか | `phase1-handoff-report.md`, `step1-3-handoff-report.md`, `pilot-test-handoff-report.md` |
...
3. **GAS URL / デプロイ状態変更時** — `.env.example` / `.env.production` / `CLAUDE.md` / `gas/README.md` / 関連 handoff を更新する。
4. **指示書パスと異なる判断** — 上記対応表と `CLAUDE.md` に理由を残す。
5. **秘密情報** — API キー・`.clasp.json` はリポジトリに置かない。
