...
最終更新: 2026-07-11（confusables/common_errors 役割分離・schema 1.2.4）
...
| 28 | `doc/ops/confusables-common-errors-role-separation.md` | 役割分離ガイドライン（量産テンプレ反映用） |
...
| `data/current/items.json` | **21 件** + Insight 公式 3・schema **1.2.4**（confusables/errors 役割分離） |
| UI | 仕様 **v3.3** + ERT トーン + v8 Mode A/B（モバイル縦スクロール・タイマー等削除） |
| EPT 参照 | `data/reference/ept/` |
| A2 本生成（2,430 件） | **未着手**（例文補充待ち・役割分離ガイドライン反映予定） |
| Phase 2 残り | SRS / Mode C / 音声・GA-RP |
...
│   ├── components/                    #   layout, modal, checkmark, ipa-tabs, insight-card
...
│   │   └── train/
│   │       ├── TrainPage.tsx
│   │       ├── mode-a/ModeAPage.tsx   #   文脈読解(v8 モバイル最適化)
│   │       ├── mode-b/ModeBPage.tsx   #   穴埋め想起
│   │       ├── hooks/useTrainInteractions.ts  # スワイプ・キーボード
│   │       └── components/
│   │           ├── TargetSidePanel.tsx
│   │           ├── EvalButtons.tsx
│   │           ├── ConfusablesInline.tsx
│   │           ├── RelatedUsesInline.tsx
│   │           └── TrainCheckmarks.tsx  # 学習履歴▢(表示のみ)
...
│   │   └── items.json                 #   現状 21 件 + insights 公式 3（schema 1.2.4）
...
| `ops/` | データ追加・GAS 変更・デプロイ・UX/i18n テスト | `data-operations-guide.md`, `claude-api-gas-design.md`, `ux-smoke-test-checklist.md`, `i18n-strategy.md`, `confusables-common-errors-role-separation.md` |
