...
最終更新: 2026-07-11（v6 UI IA・IPA/EPT・Insight）
...
| `data/current/items.json` | **21 件**（既存 11 + カテゴリパイロット 10）+ Insight サンプル 3・schema **1.2.1** |
| UI | ERT トーン + v6 IA（空セクション非表示・IPA タブ・Insight） |
| EPT 参照 | `data/reference/ept/`（wordlist / connected_speech / weak_forms） |
| A2 本生成（2,430 件） | **未着手**（21 件パイロットでの情報充足度評価待ち） |
| Phase 2 残り | SRS / Mode C / 音声・GA-RP / Insight 公式データ |
...
│   ├── features/                      #   home, train(+mode-a/b), browse, review, settings
...
│   │   └── items.json                 #   現状 21 件 + insights 3（schema 1.2.1）
...
| `src/lib/train/` | ハイライト / cloze 生成・セッションタイマー |
