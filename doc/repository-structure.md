...
最終更新: 2026-07-10（チェックマーク機能・Mode A/B 重み付き出題）
...
| `data/current/items.json` | **11 件**（サンプル 3 + パイロット v4 の 8・2026-07-10 マージ） |
| A2 本生成（2,430 件） | **未着手**（**MVP UI 実装済** → Naoya 情報充足度評価待ち） |
...
│   ├── lib/                           #   db, gas-client, i18n, quiz, checkmarks, stores
...
| `src/lib/quiz/` | Mode A distractor・`pickWeightedItem`（チェック数 8/4/2/1） |
